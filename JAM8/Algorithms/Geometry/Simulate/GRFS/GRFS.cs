...
namespace JAM8.Algorithms.Geometry
{
    ...
    public class GRFS
    {
        ...
        /// <returns></returns>
        public (Grid result, long milliseconds) run(int search_radius, double[] rot_mat, int k_cdi, int random_seed = 12312)
        {
            Stopwatch sw = new();
            sw.Start();
            ...
            //条件数据网格化
            g.add_gridProperty("cd_assign_to_grid", cd.assign_to_grid(gs).grid_assigned[property_name]);

            //计算条件数据的分位数
            Quantile quantile_cd = Quantile.create(g["cd_assign_to_grid"].buffer.
                Where(a => a != null).
                Select(a => (double)a).ToList());

            //生成高斯随机场
            g.add_gridProperty("grf", gs.dim == Dimension.D2 ?
            ...
            CData cd_loss = CData.create_from_gridProperty(g, "loss", null, false);

            //条件化
            OK ok = OK.create(gs, vm, cd_loss, "loss");//克里金插值
            var (g_ok, time) = ok.Run(search_radius, rot_mat, k_cdi);
        ...
        /// <param name="random_seed"></param>
        /// <returns></returns>
        public (Grid result, long milliseconds) run2(int search_radius, double[] rot_mat, int k_cdi, int random_seed = 123123)
        {
            Stopwatch sw = new();
            sw.Start();
            ...
            //条件数据网格化
            g.add_gridProperty("Z_0_a", cd.assign_to_grid(gs).grid_assigned[property_name]);
            CData cd_Z_0_a = CData.create_from_gridProperty(g, "Z_0_a", null, false);
            OK ok1 = OK.create(gs, vm, cd_Z_0_a, "Z_0_a");//克里金插值
            g.add_gridProperty("Z_0_ok", ok1.Run(search_radius, rot_mat, k_cdi).result[1]);

            //cd quantile 计算分位数
            Quantile quantile_cd = Quantile.create(g["Z_0_a"].buffer.Where(a => a != null).Select(a => (double)a).ToList());
            ...
            CData cd_Z_S_a = CData.create_from_gridProperty(g, "Z_S_a", null, false);
            OK ok2 = OK.create(gs, vm, cd_Z_S_a, "Z_S_a");//克里金插值
            g.add_gridProperty("Z_S_ok", ok2.Run(search_radius, rot_mat, k_cdi).result[1]);
