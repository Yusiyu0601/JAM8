...
namespace JAM8.Algorithms.Geometry
{
    ...
    public class GridProperty
    {
        ...
        /// <returns></returns>
        public GridProperty resize_nearest_to_structure(GridStructure resized_gs)
        {
        ...
        public GridProperty resize_nearest_by_scale(double scale_x = 1.0, double scale_y = 1.0, double scale_z = 1.0)
        {
            ...
            return this.resize_nearest_to_structure(resized_gs); // 调用你原来的 resize 方法
        }

        #endregion

        #region multiresolution // 放置与多分辨率相关的上采样、下采样工具函数
        ...
        public GridProperty expand_to_pow2(float outsideValue = -99f)
        {
            int next_pow2(int n)
            {
                int p = 1;
        ...
        public GridProperty pyramid_upsample_sparse(int scale_x, int scale_y, int scale_z = 1)
        {
            if (scale_x <= 0 || scale_y <= 0 || scale_z <= 0)
                throw new ArgumentException("缩放因子必须为正整数");

            var coarse_gs = this.grid_structure;
            ...
            for (int i = 0; i < coarse_gs.N; i++)
            {
                ...
                var value = this.get_value(si_coarse);
        ...
        /// 对每个 block 内有效值求平均值，跳过 null 和哨兵值，适合连续变量。
        /// </summary>
        ...
        public GridProperty pyramid_downsample_smooth(int factor_x, int factor_y, int factor_z = 1)
        {
            ...
            for (int ix = 0; ix < coarse_nx; ix++)
            {
                ...
                else
                {
                    ...
                    coarse_gp.set_value(si_coarse, average);
                }
            }
