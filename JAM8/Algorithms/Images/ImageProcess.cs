...
namespace JAM8.Algorithms.Images
{
    /// <summary>
    /// 图像处理
    /// </summary>
    public class ImageProcess
    {
        ...
        /// <returns></returns>
        public static Image from_file_faster(string fileName)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public static Image from_file_faster()
        {
        ...
        /// <returns></returns>
        public static Bitmap create_gray(int Width, int Height)
        {
        ...
        /// <returns></returns>
        public static Bitmap color_to_gray(Bitmap Src)
        {
            Bitmap b = new(Src);
            LockBitmap lockbmp = new(b);
            lockbmp.LockBits();//锁定Bitmap，通过Pixel访问颜色
            for (int i = 0; i < b.Width; i++)//循环读取像素转换灰度值
            {
                for (int j = 0; j < b.Height; j++)
                {
                    Color curColor = lockbmp.GetPixel(i, j);
                    int ret = (int)(curColor.R * 0.299 + curColor.G * 0.587 + curColor.B * 0.114);
                    lockbmp.SetPixel(i, j, Color.FromArgb(ret, ret, ret));
                }
            }
        ...
        /// <returns></returns>
        public static bool is_gray(Bitmap Bmp)
        {
        ...
        public static void wellner_adaptive_threshold(Bitmap Src, Bitmap Dest, int Radius = 5, int Threshold = 50)
        {

        }

        /// <summary>
        /// 获取灰度图像的灰度值及其统计频率，返回值为元组类型
        /// </summary>
        /// <param name="grayImage">灰度图像</param>
        /// <returns>灰度值和灰度值的频数 [元组类型]，第1个参数是灰度值集合，第2个参数是灰度值的频数</returns>
        public static (int[], int[]) get_gray_histogram(Bitmap grayImage)
        {
            List<int> grayValues = new();
        ...
        public static Image fft(Bitmap bitmap)
        {
            var fft1 = new Complex[bitmap.Width, bitmap.Height];
            Gaussian gau = new();
            for (int j = 0; j < bitmap.Height; j++)
                for (int i = 0; i < bitmap.Width; i++)
                    fft1[i, j] = new Complex(bitmap.GetPixel(i, j).R, 0);
            fft1 = MyFFT.fft2(fft1);

            GridProperty gp = GridProperty.create(GridStructure.create_simple(bitmap.Width, bitmap.Height, 1));
            for (int j = 0; j < bitmap.Height; j++)
                for (int i = 0; i < bitmap.Width; i++)
                {
                    int value = (int)fft1[i, j].Real;
                    gp.set_value(i + 1, j + 1, value);
                }
            DataMapper dm = new();
            ...
            for (int j = 0; j < bitmap.Height; j++)
                for (int i = 0; i < bitmap.Width; i++)
                {
                    int value = (int)gp.get_value(i + 1, j + 1);
                    result.SetPixel(i, j, Color.FromArgb(value, value, value));
                }

            ImageProcess.Show(result);
            return result;
        ...
        {
            Form_Image imageBox = new(bitmap);
            imageBox.ShowDialog();
        }
    }
}
