...
namespace JAM8.Algorithms.Numerics
{
    ...
    public class Gaussian
    {
        ...
        /// 随机抽取一个服从高斯分布的数值
        /// </summary>
        /// <param name="mt">The random source.</param>
        /// <returns>System.Double.</returns>
        public double Sample(MersenneTwister mt)
        {
            return Mean + STD * SampleBoxMuller(mt).Item1;
        }
        ...
        /// <returns></returns>
        public double[] sample(int N, MersenneTwister mt)
        {
        ...
        /// </summary>
        /// <param name="mt">The random number generator to use.</param>
        /// <returns>a pair of random numbers from the standard normal distribution.</returns>
        internal static Tuple<double, double> SampleBoxMuller(MersenneTwister mt)
        {
            var v1 = 2.0 * mt.NextDouble() - 1.0;
            var v2 = 2.0 * mt.NextDouble() - 1.0;
            var r = v1 * v1 + v2 * v2;
            while (r >= 1.0 || r == 0.0)
            {
