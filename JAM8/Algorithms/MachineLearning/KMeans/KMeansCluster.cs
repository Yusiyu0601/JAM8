...
{
    using Point = Coord;//Point(点)

    /// <summary>
    /// K-means cluster方法
    /// 来源：网络
    /// 网址：https://visualstudiomagazine.com/articles/2013/12/01/k-means-data-clustering-using-c.aspx
    /// </summary>
    public class KMeansCluster
    {
        ...
        /// <summary>
        /// k均值聚类，输入MyDataFrame和指定序列名称，将其聚类为N_cluseters个类别。
        /// </summary>
        /// <param name="df"></param>
        /// <param name="series_names"></param>
        /// <param name="N_clusters"></param>
        /// <returns></returns>
        public static MyDataFrame clustering(MyDataFrame df, string[] selected_series_names, int N_clusters)
        {
            double[][] input = new double[df.N_Record][];
            for (int idx_record = 0; idx_record < df.N_Record; idx_record++)//逐行计算
            {
                List<double> record = new();
                foreach (var series_name in selected_series_names)
                    record.Add(Convert.ToDouble(df[idx_record, series_name]));
                input[idx_record] = record.ToArray();//把该行数据赋给二维数组
            }

            int[] clustered = Cluster(input, N_clusters);//聚类

            var result = MyDataFrame.create(df.series_names.Append("clustered").ToArray());
            for (int idx_record = 0; idx_record < df.N_Record; idx_record++)
            {
                ...
                foreach (var series_name in df.series_names)
                    record[series_name] = df[idx_record, series_name];
                record["clustered"] = clustered[idx_record];
